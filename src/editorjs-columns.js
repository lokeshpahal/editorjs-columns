...
class EditorJsColumns {
	...
	renderSettings() {
		...
		let buttonFiveCols_Button = document.createElement("div");
		buttonFiveCols_Button.classList.add("cdx-settings-button");
		buttonFiveCols_Button.innerHTML = buttonFiveCols.icon;
		...
		buttonFiveCols_Button.addEventListener("click", (event) => {
			this._updateCols(5);
		});
		...
		wrapper.appendChild(buttonFiveCols_Button);
	...
	async _updateCols(num) {
		...
		if (num == 5) {
			this.editors.numberOfColumns = 5;
			this._rerender();
			// console.log(3);
		}
	}
